...
internal sealed class ObtenerRiesgosQueryHandler
    : IRequestHandler<ObtenerRiesgosQuery, IReadOnlyCollection<RiesgoDto>>
{
    private readonly ISigeriDbContext _context;

    public ObtenerRiesgosQueryHandler(ISigeriDbContext context)
    {
        _context = context;
    }

    public async Task<IReadOnlyCollection<RiesgoDto>> Handle(
        ObtenerRiesgosQuery request,
