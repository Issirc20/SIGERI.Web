...
internal sealed class ObtenerTratamientosPorRiesgoQueryHandler
    : IRequestHandler<ObtenerTratamientosPorRiesgoQuery, IReadOnlyCollection<PlanTratamientoDto>>
{
    private readonly ISigeriDbContext _context;

    public ObtenerTratamientosPorRiesgoQueryHandler(ISigeriDbContext context)
    {
        _context = context;
    }

    public async Task<IReadOnlyCollection<PlanTratamientoDto>> Handle(
        ObtenerTratamientosPorRiesgoQuery request,
        CancellationToken cancellationToken)
    {
        return await _context.PlanesTratamiento
            .AsNoTracking()
            .Where(p => p.RiesgoId == request.RiesgoId)
            .Include(p => p.Riesgo)
